...
class CLASSIFIER:
  #.5 awa
  #0.8 gzsl awa
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, generalized=True,ratio=0.8,epoch=45):
        ...
        #12622005634488824280
        #9182
        # torch.manual_seed(12622005634488824280)
        # if self.cuda:
        #     torch.cuda.manual_seed_all(12622005634488824280)
        if generalized:
    ...
    def fit_zsl(self):
        ...
        # with open("t_"+str(acc_fc)+".text", 'w') as f:
        #   for i in range(len(all_easy_hard_label)):
        #     f.write((str(all_easy_hard_label[i]) + ' ' + str(fc_all_pred[i])).replace('tensor(','').replace(')','') + '\n')
        sys.stdout.flush()
        return acc_fc
